...
class GemineApp:
    ...
    def create_widgets(self):
        ...
        # Vincula o "Enter" para enviar a mensagem
        self.entrada_texto.bind("<Return>", self.processar_comando)
    ...
    def gerar_resposta(self, texto):
        ...
        instrucoes = (
            f"Gerar resposta para: {texto}\n"
        )

        try:
            response = model.generate_content(instrucoes)

            # Acessa o texto corretamente
            if hasattr(response, 'candidates') and response.candidates:
                return response.candidates[0].content.parts[0].text
            else:
                return "A resposta não contém texto."
